...
class Get_Release_V1(modeltools.Method):
    ...
    Example:
        ...
        2.0
        >>> round_(model.get_release_potentialevapotranspiration_v1(1))
        4.0
    """

    REQUIREDSEQUENCES = (snow_fluxes.Snowmelt,)

    @staticmethod
    def __call__(model: modeltools.Model, k: int, /) -> float:
        flu = model.sequences.fluxes.fastaccess

        return flu.snowmelt[k]
...
class Model(modeltools.AdHocModel):
    ...
    INTERFACE_METHODS = (Determine_Release_V1, Get_Release_V1, Get_SnowCover_V1)
    OUTLET_METHODS = ()
    SENDER_METHODS = ()
    SUBMODELINTERFACES = ()
    SUBMODELS = ()
...
FluxSequence1D
        ...
        ), snow_parameters.ZipParameter1DNmbHRU.modify_refindices(
            sharable_configuration["landtype_refindices"]
        ), snow_parameters.ZipParameter1DNmbHRU.modify_refweights(
    ...
    @importtools.define_targetparameter(snow_control.HRUArea)

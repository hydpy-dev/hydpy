...
attribute 'fastaccess'
        ...
        try:
            if self.NDIM == 0:
                return ()
            try:
                return getattr(self.fastaccess, f"{self.name}_pointer").shape
            except AttributeError:
...
class LinkInSequence(LinkSequence):
    ...
    def synchronise(self) -> None:
        try:
            ...
            self.value = self.fastaccess.get_value(self.name)
        except BaseException:
            objecttools.augment_excmessage(
                f"While trying to query the value(s) of link sequence "
                f"{objecttools.elementphrase(self)}"
...
class LinkOutSequence(LinkSequence):

    def synchronise(self) -> None:
        try:
            self.fastaccess.set_value(self.name, self.value)
        except BaseException:
            objecttools.augment_excmessage(
                f"While trying to assign the value(s) {self.value} to link sequence "
                f"{objecttools.elementphrase(self)}"
            )


class InletSequence(LinkInSequence):
...
class OutletSequence(LinkOutSequence):
...
class ReceiverSequence(LinkInSequence):
...
class SenderSequence(LinkOutSequence):

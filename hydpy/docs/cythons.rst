...
The most import module (and so far the only one which is actually a
Python module) is :mod:`~hydpy.cythons.modelutils`, which implements
the routines to translate Python code into Cython code, to translate
Cython code into C code, to compile C code, and to embed the compilation
into a "Python dll" (a "pyd" file on Windows and a "so" file on Linux
systems).

The other modules are Cython extension files.  Often they are named
similar to the Python modules of other packages (see for example
:mod:`~hydpy.auxs.anntools` and :ref:`annutils`).
...
:class:`~hydpy.cythons.modelutils.Cythonizer` of module
:mod:`~hydpy.cythons.modelutils` detects a change in the code base

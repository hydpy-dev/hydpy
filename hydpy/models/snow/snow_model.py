...
class Calc_PLayer_V1(modeltools.Method):
    ...
    Basic equations:
      ...
      .. math::
        ...
        \end{cases}
        \\
        MeanPrecipitation = \sum_{i=1}^{NLayers} LayerArea \cdot PLayer*
        \\
        ...
        for k in range(con.nlayers):
            if con.zlayers[k] <= fix.zthreshold:
                flu.player[k] = inp.p * modelutils.exp(
                    con.gradp * (con.zlayers[k] - der.zmean)
                )
            elif der.zmean <= fix.zthreshold:
                flu.player[k] = inp.p * modelutils.exp(
                    con.gradp * (fix.zthreshold - der.zmean)
                )
            else:
                flu.player[k] = inp.p
...
class Return_T_V1(modeltools.Method):
    ...
    @staticmethod
    def __call__(
        model: modeltools.Model, t: float, gradtmean: float, zlayer: float
    ) -> float:
        der = model.parameters.derived.fastaccess

        return t + (der.zmean - zlayer) * gradtmean / 100.0
...
class Calc_SolidFraction_V2(modeltools.Method):
    ...
    Examples:
        ...
        At the threshold value of 1500 m, where it is switched from USACE to Turcotte
        method, discontinuities occur which vary depending on the ratio of |TLayer|
        to |TMinLayer| and |TMaxLayer|. For reasons of consistency and in agreement
        with Valery's publication (:cite:t:`ref-Valery`), we nevertheless maintain this
        distinction.
...
class Calc_PRainLayer_V1(modeltools.Method):
    ...
    CONTROLPARAMETERS = (snow_control.NLayers,)
    REQUIREDSEQUENCES = (snow_fluxes.PLayer, snow_fluxes.SolidFraction)
...
class Calc_PotMelt_V1(modeltools.Method):
    ...
    REQUIREDSEQUENCES = (snow_fluxes.TLayer, snow_states.ETG, snow_states.G)
...
class Calc_PNetLayer_V1(modeltools.Method):
    ...
    CONTROLPARAMETERS = (snow_control.NLayers,)
    REQUIREDSEQUENCES = (snow_fluxes.Melt, snow_fluxes.PRainLayer)
...
class BaseModel(modeltools.AdHocModel):
    """Base model for |snow_cn| and |snow_cn_minmax|."""

    def layers_from_hypsodata(self, hypsodata) -> None:
        """Sets |LayerArea| and |ZLayers| based on an array of length 101 (hypsodata),
        containing min, q01 to q99 and max of the catchment elevation distribution [m].

        >>> from hydpy.models.snow_cn import *
        >>> parameterstep()
        >>> nlayers(5)
        >>> model.layers_from_hypsodata(hypsodata=[
        ...     286, 309, 320, 327, 333, 338, 342, 347, 351, 356, 360, 365, 369, 373,
        ...     378, 382, 387, 393, 399, 405, 411, 417, 423, 428, 434, 439, 443, 448,
        ...     453, 458, 463, 469, 474, 480, 485, 491, 496, 501, 507, 513, 519, 524,
        ...     530, 536, 542, 548, 554, 560, 566, 571, 577, 583, 590, 596, 603, 609,
        ...     615, 622, 629, 636, 642, 649, 656, 663, 669, 677, 684, 691, 698, 706,
        ...     714, 722, 730, 738, 746, 754, 762, 770, 777, 786, 797, 808, 819, 829,
        ...     841, 852, 863, 875, 887, 901, 916, 934, 952, 972, 994, 1012, 1029,
        ...     1054, 1080,1125, 1278])
        >>> layerarea
        layerarea(0.2)
        >>> zlayers
        ...
        This works only for exactly 101 values for hypsodata:
        >>> model.layers_from_hypsodata(hypsodata=[
        ...     286, 309, 320, 327, 333, 338, 342, 347, 351, 356, 360, 365, 369, 373,
        ...     378, 382, 387, 393, 399, 405, 411, 417, 423, 428, 434, 439, 443, 448,
        ...     453, 458, 463, 469, 474, 480, 485, 491, 496, 501, 507, 513, 519, 524,
        ...
        ...     714, 722, 730, 738, 746, 754, 762, 770, 777, 786, 797, 808, 819, 829,
        ...     841, 852, 863, 875, 887, 901, 916, 934, 952, 972, 994, 1012, 1029,
        ...     1054, 1080,1125, 1278, 1300])
        Traceback (most recent call last):
        ...
        ValueError: If the hypsodata option is to be used exactly 101 values must be \
given. But 102 values were given

        If |NLayers| is not a divider of 100
        ...
        control.layerarea = 1 / control.nlayers
        nmoy = 100 // control.nlayers
        nreste = 100 % control.nlayers
        ncont = 0
        control.zlayers(numpy.nan)
        for ilayer in range(int(control.nlayers)):
            if nreste > 0:
                nn = nmoy + 1
                nreste -= 1
            else:
                nn = nmoy
            if nn <= 2:
                control.zlayers.values[ilayer] = hypsodata[ncont]
            elif nn > 2:
                control.zlayers[ilayer] = hypsodata[int(ncont + nn / 2.0)]
            ncont = ncont + nn

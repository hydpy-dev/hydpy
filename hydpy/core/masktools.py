...
class CustomMask(BaseMask):
    ...
    features, excepts that it allows its |bool| values to be defined manually.  Use it
    when you require a masking behaviour that is not captured by an available mask.

    Like the more advanced masks, |CustomMask| can either work via Python's descriptor
    protocol or can be applied directly, but is thought to be applied in the last way
    only:
    ...
    new mask, but does not change the old one (all masks are derived from
    |numpy.ndarray|):
    ...
    All features of class |numpy.ndarray| thought for |bool| values can be applied.
...
class DefaultMask(BaseMask):
    ...
    The following example shows how |DefaultMask| can be applied via Python's
    descriptor protocol, which should be the common situation:
...
class IndexMask(DefaultMask):
    ...
    are applied on the |hland| specific parameter classes
    |hland_parameters.ParameterComplete| and |hland_parameters.ParameterSoil|.  The
    documentation on the two parameter classes provides some application examples.
    Further, see the documentation on class |CustomMask| for the basic usage of class
    |DefaultMask|.
    """
    ...
    def new(cls, variable: VariableProtocol, **kwargs):
        ...
        referenced parameter are contained in the |IndexMask| class attribute tuple
        `RELEVANT_VALUES`.

        Before calling new (explicitly or implicitely), one must prepare the variable
...
as long as parameter `zonetype` is not prepared properly.
        ...
        If the shape of the |IndexMask.refindices| parameter is zero (which is actually
        not allowed for |hland|), the returned mask is empty:
    ...
    def get_refindices(cls, variable) -> "parametertools.Parameter":
        """Return the |Parameter| object for determining which entries of |IndexMask|
        are |True| and which are |False|.

        The given `variable` must be concrete |Variable| object, the |IndexMask| is
        thought for.
        ...
        Traceback (most recent call last):
        ...
        NotImplementedError: Function `get_refindices` of class `IndexMask` must be \
overridden, which is not the case for class `IndexMask`.
    ...
    def relevantindices(self) -> Set[int]:
        """A |list| of all currently relevant indices, calculated as an intercection of
        the (constant) class attribute `RELEVANT_VALUES` and the (variable) property
        |IndexMask.refindices|."""

...
|dam_v008| is a simple reservoir model, like the one provided by the "TALS"
...
Additionally, it provides means to control the stored water volume via
defining target storage values that can vary with time.  The following
examples built on the examples on application models |dam_v006| and
|dam_v007|, so we advise to read their documentation first.

Integration examples:

    We create the exact same test setting as for application models
    |dam_v006| and |dam_v007|, including an identical inflow series and
    relationship between stage and volume as well as between flood
    discharge and stage:
    ...
    In the first example, we mainly set some of the remaining parameter
    values extremely high or low, to make sure that the reservoir stores
    all water except the one activating the spillway and this becoming
    "flood discharge":
    ...
    As to be expected, reusing the the more realistic relationship between
    flood discharge and stage of the second example on |dam_v007| results
    in the same flood discharge time series:
    ...
    Application model |dam_v007| only allows to release water until the
    dam runs dry.  Application model |dam_v008| is more complex, in that
    it allows to define target storage volumes. |dam_v008| tries to control
    its outflow so that the actual volume approximately equals the
    (potentially seasonally variing) target volume.  However, it cannot
    release arbitrary low or high amounts of water to fullfill this task.
    Its first priority is not release a minimum amount of water (eventually
    due to ecological reasons).  Its second priority is not to release to
    much water (eventually due to flood protection).  In the next example,
    we activate these mechanisms through changing some related parameter
    values (also see the documentation on method |Calc_ActualRelease_V3|
    ...
    Compared with application model |dam_v007|, the flood retention
    efficiency of application |dam_v008| is less efficient.  This is due
    to target volume of 0.5 mio m³.  |dam_v007| releases all initial inflow,
    while |dam_v008| stores everything most of it.  After peak flow,
    |dam_v008| first releases water as fast as allowed, but than again
    tries to meet the target volume.  The small shift away from the targeted
    0.5 mio m³ is due to missing inflow while there is still the necessity
    to release at least 0.1 m3/s:
    ...
    expect, for example, from LARSIM.  If we set both "target range"
    parameters to zero (like one does not set the LARSIM option
    "TALSPERRE SOLLRANGE") and both "tolerance" parameters to zero (to
    disable any smoothing), be should principally get very similar results:
    ...
    The first negative water volume in the last example lies within the
    numerical error tolerance.  We can decrease such errors through setting
    other solver parameter values, but note that this might result in a
    relevant increase in computation times (especially in case one does
    apply zero smoothing values):
    ...
    LARSIM also suffers from discontinuous release values when the actual
    storage volume varies slighlty around the target value, in that it
    calculates unrealistic jumps.  This is why LARSIM implements the
    "TALSPERRE SOLLRANGE" option, which introduces linear interpolation
    between 80 % and 120 % of the target volume. |dam_v008| should not
    produce similar jumps, but also benefits from such an interpolation
    in that it operates more efficiently.  You can define the interpolation
    range freely via parameter |TargetRangeRelative| depending on your
    specific needs. In the last example, we set this value as LARSIM
    does it:

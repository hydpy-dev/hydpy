...
.. book on object-oriented design: http://www.itmaybeahack.com/homepage/books/oodesign.html
...
Please read the following hints carefully, if you intent to contribute 
...
subsections, in which we discuss the generall framework style and 
the model implementation style seperately. 
...
conventions for the development of HydPy, but is no generall guidance on how 
to write good source code.  So, if you have little experience in programming
...
compatibility HydPys code base with Python 3 is welcome!
...
usefull.  But the related import commands should be secured in
...
*********************
HydPy is intended to be applicable researchers and practitioners
who are no Python experts and maybe have generally little
experience in programming.  Hence it is desirable to anticipate
errors due to misleading input as good as possible and report them
as soon as possible.  So, in contradiction to `PEP 8`_, it is 
...
appears to be usefull. One already implemented example is that it
...
point, the computation times of the generell framework 
functionalities discussed here should be negligible in comparison
with the computation times of the hydrological simulation, 
...
thousands of times at different subcatchments.  Hence try to add
as many information to error messages as possible, e.g.::
...
(The function `devicename` tries to determine the name of the
`Node` or `Element` instance containing the given object, which
is in many cases the most relevant information for identifying
the error source.)
...
For classes like Element or Node, where names (and not namespaces)
are used to differentiate between instances, the property `name`
is also implemented, but --- of course --- not related to the 
class name, e.g.::
...
Different `Element` instances are stored in an instance of the 
class `Elements`, different `Node` instances are stored in an instance
of the class `Nodes`...

...
class NmbZones(parametertools.Parameter):
    ...
    Usually, you should first prepare |NmbZones| and then define the values of all
    1-dimensional parameters and sequences:
    ...
    sequences and requires you to reset their values:
    ...
    Redefining the same value does not delete the already available data:
...
class ZoneArea(hland_parameters.ParameterComplete):
    ...
    def trim(self, lower: TrimHook = None, upper: TrimHook = None) -> bool:
        ...
        Third, an example with zone areas that are too low:

        >>> zonearea(0.5, 1.0, 1.5)
        >>> zonearea
        zonearea(1.0, 2.0, 3.0)

        Fourth, an example with zone areas that are too high:
...
class K(parametertools.Parameter):
    ...
    Examples:
        ...
        Alternatively, you can specify the three keyword arguments mentioned above:

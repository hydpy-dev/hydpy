...
def xml_replace(filename: str, *, printflag: bool = True, **replacements: str) -> None:
    """Read the content of an XML template file (XMLT), apply the given `replacements`
    to its substitution  markers, and write the result into an XML file with the same
    name but ending with `xml` instead of `xmlt`.

    First, we write an XMLT file, containing a regular HTML comment, a readily defined
    element `e1`, and some other elements with substitution markers.  Substitution
    markers are HTML comments starting and ending with the `|` character:
    ...
    Function |xml_replace| can both be called within a Python session and from a
    command line.  We start with the first type of application.

    Each substitution marker must be met by a keyword argument unless it holds a
    default value (`e4`).  All arguments are converted to a |str| object (`e3`).
    Template files can use the same substitution marker multiple times (`e2`):
    ...
    Without custom values, |xml_replace| applies predefined default values, if
    available (`e4`):
    ...
    ...
    RuntimeError: While trying to replace the markers `e2, e3_, and e4` of the XML \
template file `test1.xmlt` with the available keywords `e2`, the following error \
occurred: Marker `e3_` cannot be replaced.
    ...
    ...
    RuntimeError: While trying to replace the markers `e2, e3_, and e4` of the XML \
template file `test1.xmlt` with the available keywords `e2, e3_, e4, and e5`, the \
following error occurred: Keyword(s) `e5` cannot be used.
    ...
    Traceback (most recent call last):
    ...
    RuntimeError: Template file `test2.xmlt` defines different default values for \
marker `e4`.

    As mentioned above, function |xml_replace| is registered as a "script function" and
    can thus be used via command line:
    ...
    try:
        ...
        objecttools.augment_excmessage(
            ...
            f"`{objecttools.enumeration(sorted(set(markers)))}` of the XML template "
            f"file `{templatename}` with the available keywords "
            f"`{objecttools.enumeration(sorted(keywords))}`"
        )

...
class Adjacency(parametertools.Parameter):
    ...
    def update(self) -> None:
        ...
        nodes = devicetools.Nodes(tuple(devicetools._registry[devicetools.Node].values()))
        elements = devicetools.Elements(tuple(devicetools._registry[devicetools.Element].values()))
        ...
        for source in source_name2element.values():
            paths = tuple(networkx.all_simple_paths(graph, target=target, source=source))
            assert len(paths) == 1
            path = paths[0]
        ...
        for source in source_name2element.values():
            for target_ in source_name2element.values():
                if source is not target_:
                    paths = tuple(networkx.all_simple_paths(graph, target=target_, source=source))
                    assert len(paths) < 2
                    if len(paths) == 1:
                        path = paths[0]
                        if all(d.name not in source_name2element for d in path[1:-1]):
        ...
        self.value = networkx.to_numpy_array(
            subgraph, nodelist=(target,) + tuple(source_name2element.values()), dtype=bool
        )[1:, :]

    @property
    def _target(self) -> devicetools.Node:
        potential_targets = [
            r for r in self.subpars.pars.model.element.receivers if str(r.variable) == "Q"
        ]

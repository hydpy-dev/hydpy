...
class Calc_PLayer_V1(modeltools.Method):
    ...
    DERIVEDPARAMETERS = (grxjland_derived.ZLayers,)

    REQUIREDSEQUENCES = (grxjland_inputs.P,)
    RESULTSEQUENCES = (grxjland_fluxes.PLayer,)
...
class Calc_TLayer_V1(modeltools.Method):
    ...
    REQUIREDSEQUENCES = (grxjland_inputs.T,)
    RESULTSEQUENCES = (grxjland_fluxes.TLayer,)
...
class Calc_Pn_En_AE_V2(modeltools.Method):
    ...
    def __call__(model: modeltools.Model) -> None:
        ...
        # d_p = sum(flu.prainlayer) / len(flu.prainlayer) +
        # sum(flu.melt) / len(flu.melt)
        d_p = 0
        for k in range(con.nsnowlayers):

...
class MixinRows(variabletools.Variable):
    """Mixin class for 1-dimensional parameters and sequence whose shape depends on the
    value of parameter |NmbRows|."""

    NDIM = 1

    def __hydpy__let_par_set_shape__(self, p: parametertools.NmbParameter, /) -> None:
        if isinstance(p, wq_control.NmbRows):
            self.__hydpy__change_shape_if_necessary__((p.value,))


class MixinRowsSectors(variabletools.Variable):
    """Mixin class for 2-dimensional parameters and sequences whose shape depends on
    the values of parameter |NmbSectors| and |NmbRows|."""
    ...
    def __hydpy__let_par_set_shape__(self, p: parametertools.NmbParameter, /) -> None:
        if isinstance(p, wq_control.NmbRows):
            ...
            rows = exceptiontools.getattr_(p.subpars.nmbrows, "value", None)
            if rows is not None:
                self.__hydpy__change_shape_if_necessary__((p.value, rows))


class MixinTrapezesSectors(variabletools.Variable):
    """Mixin class for 1-dimensional parameters and sequence whose shape depends on the
    value of parameter |NmbTrapezes| or |NmbSectors|."""

"""This module implements features which help to regularize discontinuous process
equations.
...
hydrological processes.  The related "if-else" blocks are often not theoretically
motivated.  Instead, they are thought to ease implementing ad hoc solutions of
different (parts of) process equations without taking care of the entire set of process
equations.

There are some reasons to ground new model concepts on mainly continuous process
descriptions. See e. g. `Tyralla (2016)`_ for a more exhaustive discussion of this
topic.  Nevertheless, one might often want -- at least as a starting point -- to pick
single discontinuous but well-established equations of old model concepts for a new
model concept.  The tools provided by this module can be used to regularize the
discontinuities of such equations.  More concretely, the tools are intended to replace
discontinuous process equations by continuous approximations.

Some of the implemented features are to be applied during model simulations or are in
some other way performance-critical, which is why we implement them computationally
efficient by using Cython (see the extension module |smoothutils|.
"""
...
def calc_smoothpar_logistic1(metapar: ArrayFloat) -> ArrayFloat:
    """Return the smoothing parameter corresponding to the given meta parameter when
    using |smooth_logistic1|.

    Calculate the smoothing parameter value corresponding to the meta parameter value
    2.5:
    ...
    |smooth_logistic1| differs by 1 % from the related "true" discontinuous step
    function for the input values -2.5 and 2.5 (located at a distance of 2.5 from the
    ...
    For zero or negative meta parameter values, function |calc_smoothpar_logistic1|
    zero:
...
def calc_smoothpar_logistic2(
    ...
    When using this smoothing parameter value, the output of function
    |smooth_logistic2| differs by 1% from the related "true" discontinuous step
    function for the input values -2.5 and 2.5 (located at a distance of 2.5 from the
    position of the discontinuity):
    ...
    For zero or negative meta parameter values, function |calc_smoothpar_logistic2|
    returns zero:
    ...
    Note that function |calc_smoothpar_logistic2| returns approximations only.  By
    default, it relies on linear interpolation between 10,000 unevenly spaced
    supporting points in the interval [0.0, 1,000].  The achieved interpolation
    accuracy should suffice for the anticipated applications, but be aware of low
    accuracies in the extrapolation range:
    ...
    Alternatively, you can gain higher accuracies through the iterative refinement of
    the results based on the Newton method:
    ...
    Hence, our advice to model developers is to restrict the allowed span of all
    smoothing parameters relying on function |calc_smoothpar_logistic2| to the
    mentioned interval and not to use the iterative refinement strategy.  Consider the
    refinement only in cases that require extremely high accuracies or where one needs
    to extrapolate a little (of course, the range of the supporting points, provided by
    file `support_points_for_smoothpar_logistic2.npy`, could also be extended).
    """
    if iterate:
        if metapar <= 0.0:
            return 0.0
...
def calc_smoothpar_logistic3(metapar: float) -> float:
    ...
    When using this smoothing parameter value, the output of function
    |smooth_logistic3| would ideally differ by 1% from the related "true" discontinuous
    step function for the input values -2.5 and 3.5 (located at a distance of 2.5 from
    the position of the nearest discontinuity):
    ...
    |smooth_logistic3| is not in perfect agreement with the given meta parameter.  For
    most purposes, the resulting error is negligible.  If you strive for very high
    accuracy, ask us to implement some iterative refinement strategy or try it on your
    own.
    """
...
def calc_smoothpar_max1(metapar: float) -> float:
    ...
    When using this smoothing parameter value, the output of function |smooth_max1| is
    0.01 above the discontinuous maximum function result, if the absolute value of the
...
def calc_smoothpar_min1(metapar: float) -> float:
    ...
    When using this smoothing parameter value, the output of function |smooth_min1| is
    0.01 below the discontinuous minimum function result, if the absolute value of the
    difference between the x and the y value is 2.5:
